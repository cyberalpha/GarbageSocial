...
// Which default log path to use?
$log_path  = JPATH_SITE.'/log';
if(is_dir(JPATH_SITE.'/logs')) {
	if(is_writeable(JPATH_SITE.'/logs')) {
		// "logs" exists and is writable
		$log_path = JPATH_SITE.'/logs';
	} else {
		// "logs" exist, is not writable, but "log" does not exist either
		if(!is_dir($log_path)) {
			$log_path = JPATH_SITE.'/logs';
		}
	}
...
<script type="text/javascript" language="javascript">//<![CDATA[
	...
	abi_default_log = '<?php echo addslashes($log_path) ?>';
//]]></script>
