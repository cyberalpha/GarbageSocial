...
$list = ModSobiproCategoriesHelper::getList2Level($parentid, $count, $sorder, $require_stats, $debug);
if (!count($list))
{
	echo 'Empty Section';
	return;
...
<ul class="sobipcateg<?php echo $params->get('moduleclass_sfx'); ?>">
	...
	while ($i < count($list))
	{
		...
		$urlparams = array('sid' => $catid, 'title' => $list[$i]->c1name);
		if ($iid)
		{
		...
		{
			echo "\t<li class=\"sobipcateg_l1 {$evenoddd}{$class}\"><a href=\"{$url}\" class=\"{$class}\">{$list[$i]->c1name}</a>{$outpcount}\n";
		...
		while (($i < count($list)) && ($catid == $list[$i]->c1id))
		{
			...
			if ((!$hide_empty) || ($total_l1 > 0))
			{
				...
				$urlparams = array('sid' => $myId, 'title' => $list[$i]->name);
				if ($iid)
				{
					$urlparams['Itemid'] = $iid;
				}
				...
				{
					echo "\t\t<li class=\"sobipcateg_l2 {$evenoddd}{$class}\"><a href=\"{$url}\" class=\"{$class}\">{$list[$i]->name}</a>{$outpcount}</li>\n";
				}
			}
			$i++;
		}
