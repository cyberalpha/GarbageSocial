...
(function(){
    BT.MarkerList = new Class({
        ...
        initialize:function(options){
            ...
            var markers =JSON.decode(new BT.Base64().base64Decode(this.options.encodedItems));
            var self=this;
            ...
            document.adminForm.onsubmit=function(){
                ...
                $("btg-hidden").set("value",new BT.Base64().base64Encode(JSON.encode(markers)));
                if(submit!=null){
                    submit()
                }
            }
         ...
         // this code will setup show/ hide form, bind event when create/update marker
            ...
            if(!edit){
                ...
                jQ(self.options.btnCreateID).unbind('click').bind('click', function(){
                    ...
                    marker.markerShadowImage = jQ('#btg-shadow-image').val();
                ...
                scrolltoTop = jQ("#btnAddMarker").position().top / 2 ;
                if(jQ(window).width()<480){
                	scrolltoTop =jQ("#btnAddMarker").position().top;
                }
                if( parseInt(jQ("#btnAddMarker").attr('versionjl'),10) == 3){
                	jQ('html,body').animate({scrollTop: scrolltoTop + 40},'fast');
                }else{
                	jQ('html,body').animate({scrollTop: jQuery("#markers-manager-options").position().top},'fast');
                }
                ...
                if(!marker.markerShadowImage == ''){
	                jQ('#btg-shadow-image').val(marker.markerShadowImage);
	                jQ('#btg-shadow-image_preview').attr('src','../'+marker.markerShadowImage);
	                jQ('#btg-shadow-image_preview_empty').hide();
	                jQ('#btg-shadow-image_preview_img').show();
                }
        ...
        update: function(li, marker){
            ...
            shadowImage = jQ('#btg-shadow-image').val();
