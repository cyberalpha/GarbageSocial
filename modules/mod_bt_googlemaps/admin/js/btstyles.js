...
(function(){
    BT.StyleList = new Class({
        ...
        initialize:function(options){
            ...
            var styles =JSON.decode(new BT.Base64().base64Decode(this.options.encodedItems));
            var self=this;
            ...
            document.adminForm.onsubmit=function(){
                ...
                $("btg-style-hidden").set("value",new BT.Base64().base64Encode(JSON.encode(styles)));
                if(submit!=null){
                    submit()
                }
            }
           ...
           if(jQ("#btg-style-dialog").length >0){
            ...
            if(!edit){
                ...
                */
                ...
                scrolltoTop = jQ("#btnAddStyle").position().top / 2 ;
                if(jQ(window).width()<480){
                	scrolltoTop =jQ("#btnAddStyle").position().top;
                }
                if( parseInt(jQ("#btnAddStyle").attr('versionjl'),10) == 3){
                	jQ('html,body').animate({scrollTop: scrolltoTop + 40},'fast');
                }else{
                	jQ('html,body').animate({scrollTop: jQuery("#styles-manager-options").position().top},'fast');
                }
