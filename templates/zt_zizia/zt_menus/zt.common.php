...
class MenuSystem
{
	...
	$xdelay = 350, $xduration = 350, $xtransition = 'Fx.Transitions.linear')
	{
		...
		$this->mega			= '<script type="text/javascript"> /*<![CDATA[ */ window.addEvent("domready", function(){var _smart = Array();var megas = $(document.body).getElements(\'div[class="menusub_mega"]\');megas.each(function(mega, i){var id = mega.getProperty("id").split("_");if(id[2] != null){var smart = "_" + id[1] + "_" + id[2];_smart.include(smart); }});_smart.each(function(item, index){ZTMenu('.$xdelay.', 0, 0, item, "megamenu_close", true, '.$xduration.', '.$xtransition.');});}); /* ]]> */ </script>';
		$this->moo			= '<script type="text/javascript"> /*<![CDATA[ */ window.addEvent("domready",function() {new MooMenu($("menusys_moo"), {transition: '.$xtransition.', duration: '.$xduration.'})}); /* ]]> */ </script>';
		$this->drill		= '<script type="text/javascript"> /*<![CDATA[ */ var mymenu=new drilldownmenu({menuid: "drillmenu1", base: "'.JURI::base().'", menuheight: "auto", breadcrumbid: "drillcrumb", persist: {enable: true, overrideselectedul: true}, base: "'.JURI::base().'templates/'.$this->_template.'/zt_menus/'.'"}) /* ]]> */ </script>';
		...
		if($fancy) {
			...
			$this->fancy = '<script type="text/javascript">window.addEvent("load", function(){new ZTFancy($("menusys_'.$name.'"), {transition: '.$transition.', duration: '.$duration.', onClick: function(ev, item){ev.stop();}});});</script>';
		}
	...
	function _showMenuDetail($row, $level = 0)
	{
		...
		$nofollow		= ($menu_params->get('mega_nofollow')) ? " rel=\"nofollow\"" : "";
		...
		if(@$row->url != null)
		{
			if($row->browserNav == 0)
			{
				$menuItem = '<a href="'.$row->url.'" '.$Class.' '.$id.' '.$title.$nofollow.'>'.$str.'</a>';
			}
			elseif($row->browserNav == 1)
			{
				$menuItem = '<a target="_blank" href="'.$row->url.'" '.$Class.' '.$id.' '.$title.$nofollow.'>'.$str.'</a>';
			}
			elseif($row->browserNav == 2)
			{
				...
				$menuItem 	= '<a href="'.$url.'" onclick="window.open("'.$url.'",\'targetWindow\',\''.$atts.'\'); return false;" '.$Class.' '.$id.' '.$title.$nofollow.'>'.$str.'</a>';
			}
		}
		else
		{
			$menuItem = '<a '.$id.' '.$title.$nofollow.'>'.$str.'</a>';
		}
		echo $menuItem;
	}
	...
	{
		return(in_array($menu_item->id, $this->_active)) ? " class='item active'" : " class=' item'";
	}
	...
	*/
	...
	function showMegaMenu($per, $pid, $level)
	{
		if(@$this->_nav[$pid])
		{
			$this->beginUl( "level0", "menusys_".$this->_name);
			$i = 0;
			foreach($this->_nav[$pid] as $menu) 
			{
				$params	= $menu->params;
				...
				$class .=($this->isActive($menu)) ? " active" : " item";
				$class .=($aclass != '') ? $aclass : "";
		...
		*/
		for($i = 1; $i <= $cols; $i++)
		{
			...
			if($group)
			{
				for($g = 0; $g < $count; $g++)
				{
					$this->beginDiv("megacol column$i  $class", NULL, $width);
	...
	function genMegaTypeNormal($row, $level = 0)
	{
		...
		$nofollow		= ($this->getMegaMenuParam($params, "mega_nofollow", 0)) ? " rel=\"nofollow\"" : "";
