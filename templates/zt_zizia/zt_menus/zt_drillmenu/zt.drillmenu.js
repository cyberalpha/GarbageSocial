function drilldownmenu(setting){
	this.sublevelarrow={src: setting.base + 'zt_drillmenu/images/right.gif', width:'6px', top:'3px', left:'6px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow= setting.base +  'zt_drillmenu/images/right.gif' //full URL to image separating breadcrumb links (if it's defined)
	this.loadingimage= setting.base +  'zt_drillmenu/images/loader.gif' //full URL to 'loading' image, if menu is loaded via Ajax
	this.homecrumbtext='Home' //Top level crumb text
	this.titlelength=35 //length of parent LI text to extract from to use as crumb titles
	this.backarrow= setting.base +  'zt_drillmenu/images/leftarrow.gif' //full URL to image added in front of back LI
	...
	this.arrowposx=0 //zana arrow x position relative to parent LI
	var thisdrill=this
	jQuery.noConflict()
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.arrowposx=parseInt(this.$arrowimgs.eq(0).css('left')) //get zana x position of arrow
	if (window.opera)
	...
	else if (selectedulcheck){ //if "selectedul" setting defined, slide to that UL by zana
...
drilldownmenu.prototype.updatenav=function($, endorder){
	...
	if (this.navdivs.$backbuttons.length>0){ //if back buttons found
		if	(!/Safari/i.test(navigator.userAgent)) //exclude Safari from button state toggling due to bug when the button is an image link
			this.navdivs.$backbuttons.css((endorder>0)? {opacity:1, cursor:'pointer'} : {opacity:0.5, cursor:'zana'})
	}
}
