...
(function(jQuery){
    ...
    jQuery.fn.buildVertexToggle = function() {
        return jQuery(this).find('.vItem').find('input:radio:first').each(function () {
            ...
            var b = a.attr('id');
            if (b.length) {
                jQuery('input#' + b).vertexToggle({
                    onClickOff: function () {
                        jQuery(this).parent('div').find('input:last').attr('checked', 'checked')
                    }
                });
            }
        });
    ...
    jQuery.fn.buildVertexModuleSlider = function(autoSelectVal, manualSelectVal) {
        return jQuery(this).find('.vItem').find('textarea[id$="manual_widths"]').each(function (i) {
            ...
            var slider = jQuery('<div />').attr('id', id + '_slider').slider({
                range: true,
                min: 0,
                max: 100,
                step: 1,
            ...
            slider_adv.bind('click', function () {
                var input1 = jQuery('<input />').addClass('inline-input').attr({size: 2}).val(currentVal[0]).css({'margin-top': 5}).hide();
                var input2 = jQuery('<input />').addClass('inline-input').attr({size: 2}).val(currentVal[1]).css({'margin-top': 5}).hide();
                var input3 = jQuery('<input />').addClass('inline-input').attr({size: 2}).val(currentVal[2]).css({'margin-top': 5}).hide();
                var input4 = jQuery('<input />').addClass('inline-input').attr({size: 2}).val(currentVal[3]).css({'margin-top': 5}).hide();
                var input5 = jQuery('<input />').addClass('inline-input').attr({size: 2}).val(currentVal[4]).css({'margin-top': 5}).hide();
                var input6 = jQuery('<input />').addClass('inline-input').attr({size: 2}).val(currentVal[5]).css({'margin-top': 5}).hide();
...
jQuery(document).ready(function() {
  ...
  var s5_hide_component_items = jQuery('#xml_s5_hide_component_items');
  jQuery.ajax({
    type: 'GET',
    url: json_path,
    dataType: 'json',
    ...
    success: function(data) {
      var this_style = jQuery('#jform_title').val();
      jQuery.each(data.vertexFramework, function(k, v){
        if(k == this_style) {
          jQuery(s5_hide_component_items).find('option').map(function(i){
            var array_val = jQuery(this).val();
            var array_text = jQuery(this).text();
            s5_hide_component_items_array[array_val] = {};
            s5_hide_component_items_array[array_val].text = array_text;
            s5_hide_component_items_array[array_val].selected = false;
            var vals = v.xml_s5_hide_component_items;
            if(vals) {
              vals = vals.split(',');
              var isinit = jQuery.inArray(array_val, vals);
              if(isinit > -1) {
                s5_hide_component_items_array[array_val].selected = true;
              }
            }
          });
        } else {
  ...
  var style_form = jQuery('#style-form').css({width: '50%', 'float': 'left'}).find('.width-60').removeClass('width-60');
  var vertex_form = jQuery('#style-form').find('.width-40').html();
  var old = jQuery('#style-form').find('.width-40');
  var vertex_form_new = old.find('#vertex_fader');
  jQuery(old).remove();
  var new_admin = jQuery('<div />').attr('id', 'vertex_admin_wrap').css({width: '50%', 'float': 'left'});
  jQuery('#style-form').after(new_admin);
  new_admin.wrap(jQuery('<form />').attr({id: 'vertex_admin_form', method: 'post', action: ''}).addClass('vertex-admin-form'));
  ...
  function doPost() {
    ...
    jQuery.ajax({
      ...
      async: false,
      dataType: 'json',
      data: {vertex: jQuery(form).serializeArray(), style: jQuery('#jform_template').val(), style_name: style_name},
  ...
  jQuery('#toolbar-apply').children().click(function(){
    var res = doPost();
    if(res) javascript:Joomla.submitbutton('style.apply');
    return false;
  });
  jQuery('#toolbar-save').children().click(function(){
    var res = doPost();
    if(res) javascript:Joomla.submitbutton('style.save');
    return false;
  });
  jQuery('#toolbar-save-copy').children().click(function(){
    var res = doPost();
  ...
  jQuery('#vertex_admin_wrap').find('.vItem').each(function(){
    ...
    if(forattr == 's5_menu_type') {
      jQuery(this).children('.vItemValue').append(s5_menu_type);
    }
    if(forattr == 's5_hide_component_items') {
      jQuery(this).children('.vItemValue').append(s5_hide_component_items);
    }
  });
  setTimeout(function(){
