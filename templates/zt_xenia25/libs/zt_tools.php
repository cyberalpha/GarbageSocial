...
class ZT_Tools {
	...
	*/
	 ...
	 * @return string
	 */
	function getPageClassSuffix($Itemid = 0) {
		$db = &JFactory::getDBO();
		if ($Itemid==0) {
		...
		$db->setQuery($sql);
		$p = $db->loadObject();
		$p = new JParameter($p->params);
		return $p->get('pageclass_sfx');
	}
	
	function getParamsValue($prefix, $type, $key)
	{
		$document = &JFactory::getDocument();
		
		$val = $this->get_cookie($prefix.'_'.$type.'_'.$key);
		if($val == '') $val = $this->getParam($type.'_'.$key);
