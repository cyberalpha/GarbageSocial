	...
	smartBoxes.each(function(item, i)
	{
		...
		$(currentBox).addEvent('mouseenter', function()
		{
			...
			if(item.getProperty('id').split("_")[2] == 'sub0') {
				...
				if((boxSize.x - inputSize.x) < leftOffset) { item.setStyle('left', leftOffset + inputSize.x - boxSize.x); }
				else{ item.setStyle('left', leftOffset); };
			} else {				
				if(inputLeftPOS > boxSize.x) {
					var space = (WindowX - inputLeftPOS) - boxSize.x - inputSize.x*2 - 10;
					var right  = (space > inputSize.x) ? space : inputSize.x;
					item.setStyle('right', right);
				} else {
					var space = inputRightPOS - inputSize.x/2 + 10;
					var right = (space > inputSize.x) ? space : inputSize.x;
					item.setStyle('left', right);
				};
			}
