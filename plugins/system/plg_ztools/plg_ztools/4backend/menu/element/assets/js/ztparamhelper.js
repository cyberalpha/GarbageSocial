...
var ztparamhelper_zt = new FormController_ZT();

function ztph_addgroup_zt (control, options) {
	ztparamhelper_zt.add (control, options);
}

window.addEvent('load', function() {
	ztparamhelper_zt.start.delay (100, ztparamhelper_zt);
});
...
function updateFormMenu(obj, changeHeight){
	...
	switch(obj.value.trim()){
		case '0':
			...
			$('jform_params_mega_module_style').getParent().setStyle('display', 'none');
			break;
		case 'mod':
			$('jformparamsmega_subcontent_mod_modules').getParent().setStyle('display', 'block');
			$('jformparamsmega_subcontent_pos_positions').getParent().setStyle('display', 'none');
			$('jform_params_mega_module_style').getParent().setStyle('display', 'block');
			break;
		case 'pos':
			$('jformparamsmega_subcontent_mod_modules').getParent().setStyle('display', 'none');
