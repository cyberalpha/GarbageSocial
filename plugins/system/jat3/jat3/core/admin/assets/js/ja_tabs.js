...
var JATabs = new Class({	
	initialize: function(element, options) {
		...
		this.titles[this.options.numbtab-1].addClass ('active');
		...
		if(this.options.activateOnLoad != 'none')
		{
			...
			{
				this.activate(this.titles[this.options.numbtab-1],  true);				
			}
			else
			{
	...
	_getContent: function(tab){
		...
		var imgloading = new Element('img', {'src': 'plugins/content/jatabs/jatabs/loading.gif','width': 13});
