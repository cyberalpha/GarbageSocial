...
window.addEvent('load', function (e){
	...
	$('ja-tabswrapmain').getElements('.t3anchor-hide').each(function(el){
		el.getParent().getParent().addClass('hidden');
	});

		//joomla 3.0 compatible
		var j30option = $('style-form').getElement('.nav-tabs');
		if(j30option){
			j30option.getElements('a').each(function(el){
				if((typeof el.hash != 'undefined' ? el.hash : el.href.replace(/.*(?=#[^\s]*$)/, '')) == '#options'){
			...
			setTimeout(function(){
				$$('div.ja-popup-blocks, div.ja-popup-positions, div.ja-popup-profiles, div.ja-popup-pageids, div.ja-popup-themes').each(function(elm){
					new Element('div', {'class': 'jp-inner'}).adopt(elm.getChildren()).inject(elm);
				});	
			}, 1700);
		}

		var blockright = $$('#style-form .width-60')[1];
		if(!blockright){
			blockright = $('assignment');
		}
		
		if(blockright){
			blockright.set('text', '');
			$('ja-page-assignment').inject(blockright);		
		}
		//end joomla 3.0 compatible
		...
		// Button help
		$('toolbar-help').getChildren().destroy();
...
<div class="container" id="ja-tabswrap">
	<div style="height: 30px;" class="ja-tabs-title-top">
		...
		<a class="ja-icon-video" href="javascript:void(0)" onclick="$('ja-introduce').fireEvent('click');"><span><?php echo JText::_('Video')?></span></a>
	</div>
...
var lg_confirm_delete_profile_assiged = '<?php echo JText::_('ARE_YOU_SURE_TO_DELETE_THIS_PROFILE_ASSIGNED')?>';
...
var lg_convert_folder_success = '<?php echo JText::_('CONVERT_SUCCESS_MESSAGE'); ?>';
	...
	var table_params = $('templatestyleOptions');
	var blockleft = $('style-form').getElement('div.width-40');
	if(blockleft){
		table_params = blockleft.getFirst();
	}

	var tabs = $('ja-tabswrapmain');
	if(table_params){
		tabs.inject(table_params, 'before');
		// Fix bug show extra panels
		//if(table_params.getElement('div.panel')!=null){
		//    table_params.getElement('div.panel').inject($('jat3-profile-params'));
		//}
		expand_panel = table_params.getElements('div.panel');
		if (expand_panel.length > 0) {
			for(i = 0; i < expand_panel.length; i++) {
				expand_panel[i].inject($('jat3-profile-params'));
			}
		}
