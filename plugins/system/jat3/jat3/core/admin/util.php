...
class JAT3_AdminUtil
{
    ...
    */
        ...
        if (@is_dir($path)) {
            ...
            foreach ($file_list as $file) {
                ...
                if (@file_exists($filepath)) {
					...
					$params = JFile::read($filepath);
					if (!(JFile::read($filepath) instanceof JRegistry))
					{
						$params =  new JRegistry(JFile::read($filepath));
					}
        ...
        if ($file_profiles) {
            foreach ($file_profiles as $name=>$p) {
                ...
                if (JFile::exists($file)) {
					...
					// Strict standards: Declaration of JParameter::loadSetupFile() should be compatible with that of JRegistry::loadSetupFile() 
					$params =  JFile::read($filepath);
					if (!(JFile::read($file) instanceof JRegistry))
					{
						$params =  new JRegistry(JFile::read($file));
					}
     ...
     */
    public function getTemplateVersion($template)
    {
        $version = '';
        $name = '';
     ...
     * @return bool  FALSE
     */
    public static function checkCondition_for_Menu()
    {
        return false;
    }
     ...
     * @return void
     */
    public static function loadScipt()
    {
        if (JRequest::getCmd('option', '') == 'com_templates'
            && JRequest::getCmd('layout') == 'edit'
            && JRequest::getCmd('view') == 'style'
