...
<div class="ja-box-usertools">
  ...
  <?php foreach ($themes as $theme=>$themeinfo) : ?>
    <li class="theme theme-<?php echo str_replace('.','-',$theme).(in_array($theme, $currthemes)?'-active':'') ?>">
      ...
      <label for="user_themes_<?php echo $theme ?>" title="<?php echo $this->getInfo ($themeinfo, 'description')?$this->getInfo ($themeinfo, 'description'):$theme ?>">
        <span><?php echo $this->getInfo ($themeinfo, 'name')?$this->getInfo ($themeinfo, 'name'):$theme ?></span>
      </label>
    </li>
  <?php endforeach; ?>
  </ul>
