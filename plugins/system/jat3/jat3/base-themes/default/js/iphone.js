...
var JAIToolbox = new Class({
    ...
    start: function() {
        ...
        this._overlay = $('ja-overlay');
        this._mainbox = $('ja-toolbar-main');
        if (this.options.animOn) {
            this._boxes.setStyle ('opacity', 0);
            this._overlay.setStyle ('opacity', 0);
            this._boxes.push ($('ja-toolbar-main'));
    ...
    togglebox: function (link) {
        ...
        if (this._currenttoggle == null) {
            //show overlay
            this._overlay.setStyles ({'display':'block', 'height':$('ja-wrapper').offsetHeight});
        }
        this.showbox (link, true);
