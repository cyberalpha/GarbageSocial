...
/* Created at: Thu Jan 17 23:43:41 ART 2013 */
...
// ========
// File: /components/com_sobipro/lib/js/adm/sobipro.js
// ========
 ...
 * $Revision: 551 $
 * $Author: Radek Suski $
 * File location: components/com_sobipro/lib/js/adm/sobipro.js $
 */
 ...
 function SPResetCount( type )
 {
	 if( SP_id( 'sp_counter' ).value && SP_id( type + '.id' ).value ) {
		 SobiPro.Request( SobiProAdmUrl.replace( '%task%', type + '.resetCounter' ) + '&sid=' + SP_id( type + '.id' ).value +'&format=raw',  { method: 'get', onComplete: function() { SP_id( 'sp_counter' ).value = 0; } }  ).request();				 
	 }
 }
 window.addEvent( 'domready', function() { try { $( 'general.show_pb_no' ).addEvent( 'click', function() { if( $( 'general.show_pb_no' ).checked ) SobiPro.Alert( 'PBY_NO' ); } ); } catch (e) {}; } );
