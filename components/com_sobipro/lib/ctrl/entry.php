 ...
 * $Revision: 2404 $
 * $Author: Sigrid Suski $
 * File location: components/com_sobipro/lib/ctrl/entry.php $
 */
...
class SPEntryCtrl extends SPController
{
    ...
    private function approve()
    {
        if ( $this->_model->isCheckedOut() ) {
            Sobi::Redirect( Sobi::Back(), Sobi::Txt( 'EN.IS_CHECEKD_OUT' ), SPC::ERROR_MSG, true );
        }
    ...
    private function state( $state )
    {
        ...
        if ( ( ( $this->_model->get( 'owner' ) == Sobi::My( 'id' ) ) && Sobi::Can( 'entry.publish.own' ) ) || Sobi::Can( 'entry.publish.*' ) ) {
            ...
            Sobi::Redirect( Sobi::GetUserState( 'back_url', Sobi::Url( array( 'sid' => $this->_model->get( 'id' ) ) ) ), $state ? Sobi::Txt( 'EN.PUBLISHED' ) : Sobi::Txt( 'EN.UNPUBLISHED' ) );
        }
        else {
            Sobi::Error( 'entry', SPLang::e( 'UNAUTHORIZED_ACCESS' ), SPC::ERROR, 403, __LINE__, __FILE__ );
        }
