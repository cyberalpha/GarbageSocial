 ...
 * $Revision: 2326 $
 * $Author: Radek Suski $
 * File location: components/com_sobipro/lib/ctrl/adm/config.php $
 */
...
class SPConfigAdmCtrl extends SPController
{
    ...
    public function execute()
    {
        ...
        switch ( $this->_task ) {
            case 'clean':
                ...
                Sobi::Redirect( Sobi::Back(), Sobi::Txt( 'MSG.CACHE_CLEANED' ), null, true );
                break;
            default:
    ...
    protected function view()
    {
        ...
        if ( SPLoader::path( 'config.' . $this->_task, 'adm', true, 'ini', false ) ) {
            ...
            $view->setCtrl( $this );
            $view->loadConfig( 'config.' . $this->_task );
            $view->setTemplate( 'config.default' );
            $view->assign( SPLoader::loadIniFile( 'config.fields.' . $this->_task, true, true ), 'fields' );
    ...
    public function getNameFields( $pos = false, $types = array() )
    {
        ...
        /*static */$cache = array( 'pos' => null, 'npos' => null );
        /**
        ...
        $fields = array();
        if ( count( $fids ) ) {
            $model = SPLoader::loadModel( 'field', true );
            foreach ( $fids as $fid ) {
                $f = new $model();
                $f->init( $fid );
                $fields[ $fid ] = $f;
    ...
    protected function save()
    {
        ...
        $section = false;
        // strange thing =8-O
        if ( !( isset( $_POST[ 'spcfg_alphamenu_extra_fields_array' ] ) ) ) {
            $_POST[ 'spcfg_alphamenu_extra_fields_array' ] = array();
        }
        foreach ( $_POST as $k => $v ) {
            if ( strstr( $k, 'spcfg_' ) !== false ) {
                $v = SPRequest::raw( $k, null, 'POST' );
                if ( is_string( $v ) ) {
        ...
        if ( $section ) {
            $sMod = SPLoader::loadModel( 'section' );
            if ( $sMod ) {
                /* @var $sec SPSection */
                $sec = new $sMod();
        ...
        /* @TODO: define language path in Joomla and copy files if not exist */
        if ( SPRequest::word( 'spcfg_language_domain' ) ) {
        ...
        try {
            ...
            Sobi::Error( $this->name(), SPLang::e( 'CANNOT_SAVE_CONFIG_DB_ERR', $x->getMessage() ), SPC::WARNING, 500, __LINE__, __FILE__ );
        }
        SPFactory::cache()->cleanSection();
        Sobi::Trigger( 'After', 'SaveConfig', array( &$values ) );
        Sobi::Redirect( SPMainFrame::getBack(), Sobi::Txt( 'MSG.CONFIG_SAVED' ) );
    }
}
