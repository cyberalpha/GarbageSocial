 ...
 * $Revision: 2318 $
 * $Author: Radek Suski $
 * File location: components/com_sobipro/lib/mlo/input.php $
 */
...
abstract class SPHtml_Input
{
	...
	{
		$params = self::params( $params );
		$value = strlen( $value ) ? str_replace( '"', '&quot;', SPLang::entities( /*Sobi::Txt*/( $value ), true ) ) : null;
	...
	public static function submit( $name, $value = null, $params = null )
	{
		...
		$value = strlen( $value ) ? SPLang::entities( /*Sobi::Txt*/( $value ), true ) : null;
...
//					$vt = SPLang::entities( /*Sobi::Txt*/htmlentities( $v ), true );
					...
					if( count( $l ) ) {
						foreach ( $l as $ov => $ol ) {
							if( is_array( $ol ) ) {
								self::optGrp( $cells, $selected, $ol, /*Sobi::Txt*/( $ov ) );
							}
							else {
...
//					}
    ...
    private function optGrp( &$cells, $selected, $grp, $title )
	{
		...
		foreach ( $grp as $v => $l ) {
			...
			if( is_array( $l ) ) {
				self::optGrp( $cells, $selected, $l, /*Sobi::Txt*/( $v ) );
			}
			else {
				$sel = in_array( ( string ) $v, $selected, true ) ? ' selected="selected" ' : null;
