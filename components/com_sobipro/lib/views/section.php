<?php
/**
 * @version: $Id: section.php 2482 2012-06-18 16:07:34Z Radek Suski $
 ...
 * $Revision: 2482 $
 * $Author: Radek Suski $
 * File location: components/com_sobipro/lib/views/section.php $
 */
...
class SPSectionView extends SPFrontView implements SPView
{
                ...
                if ( count( $subcats ) ) {
                    foreach ( $subcats as $id => $name ) {
                        $sc[ ] = array(
                            '_complex' => 1,
                            '_data' => $name,
                            '_attributes' => array( 'lang' => Sobi::Lang( false ), 'id' => $id, 'url' => Sobi::Url( array( 'title' => $name, 'sid' => $id, ) ) )
                        );
                    }
                }
    ...
    private function view()
    {
        ...
        if ( $type == 'xslt' ) {
            ...
            $cUrl = array( 'title' => $current->get( 'name' ), 'sid' => $current->get( 'id' ) );
