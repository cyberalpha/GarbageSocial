...
if (JRequest::getVar( 'print', '', '', 'int' ) == 1 || JRequest::getVar( 'tmpl', '', '', 'string' ) == 'component') {
	...
	echo '<div id="phocamaps" class="phocamaps'.$this->params->get( 'pageclass_sfx' ).'">';
} else {
	echo '<div id="phocamaps" class="phocamaps'.$this->params->get( 'pageclass_sfx' ).'">';
...
if ((!isset($this->map->longitude))
	...
	if ($this->tmpl['displaydir']) {
		...
		if ($form != '') {
			...
			echo JText::_('COM_PHOCAMAPS_FROM_ADDRESS').': <input type="text" size="30" id="fromPMAddress'.$id.'" name="pmfrom'.$id.'" value=""/>';
			echo $form;
			echo ' <input name="pmsubmit'.$id.'" type="submit" value="'.JText::_('COM_PHOCAMAPS_GET_ROUTE').'" /></form></div>';
	...
	echo $map->addAjaxAPI('maps', '3', $this->tmpl['params']);
	...
	//	echo $map->setMapOption('continuousZoom', $this->map->continuouszoom).','."\n";// Not ready yet
		...
		if ($this->tmpl['close_opened_window'] == 1) {
		...
		if (isset($this->marker) && !empty($this->marker)) {
			...
			foreach ($this->marker as $key => $markerV) {
				...
				&& (isset($markerV->latitude) && $markerV->latitude != '')) {
					...
					echo $map->setMarker($markerV->id,$markerV->title,$markerV->description,$markerV->latitude, $markerV->longitude, $iconOutput['icon'], $iconOutput['iconid'], $text, $markerV->contentwidth, $markerV->contentheight,  $markerV->markerwindow, $iconOutput['iconshadow'], $iconOutput['iconshape'], $this->tmpl['close_opened_window'] );
