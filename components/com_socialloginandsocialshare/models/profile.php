...
class SocialLoginAndSocialShareModelProfile extends JModelForm
{
	...
	public function delmap($mapProvider, $map_userid)
	{
		$db =& JFactory::getDBO();
	    $sql = "DELETE FROM #__LoginRadius_users WHERE provider = " . $db->Quote ($mapProvider)." AND LoginRadius_id = " .$db->Quote ($map_userid) . " AND id = ". JFactory::getUser()->id;
		$db->setQuery ($sql);
		if ($db->query()) {
		  return true;
	    }
