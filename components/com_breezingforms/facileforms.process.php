            ...
            if ($row->type == "Captcha") {
				...
				function bfCheckCaptcha(){
					if(checkFileExtensions()){
                                               ...
                                               ao.sndReq("get","' . JURI::root(true) . ( JFactory::getApplication()->isAdmin() ? '/administrator/' : (JFactory::getConfig()->getValue('config.sef') && !JFactory::getConfig()->getValue('config.sef_rewrite') ? '/index.php/' : '/').(JRequest::getCmd('lang','') && JFactory::getConfig()->getValue('config.sef') ? JRequest::getCmd('lang','') . '/'  : '') ) . '?lang='.JRequest::getCmd('lang','').'&raw=true&option=com_breezingforms&checkCaptcha=true&Itemid=' . JRequest::getInt('Itemid', 0) . '&value="+document.getElementById("bfCaptchaEntry").value,"");
					}
				}';
                break;
                ...
                $capFunc = 'var bfReCaptchaLoaded = true;
					...
					if(checkFileExtensions()){
                                                function bfValidateCaptcha()
                                                {
                                                    ...
                                                    type: "POST",
                                                    url: "' . JURI::root(true) . ( JFactory::getApplication()->isAdmin() ? '/administrator/' : (JFactory::getConfig()->getValue('config.sef') && !JFactory::getConfig()->getValue('config.sef_rewrite') ? '/index.php/' : '/').(JRequest::getCmd('lang','') && JFactory::getConfig()->getValue('config.sef') ? JRequest::getCmd('lang','') . '/' : '') ) . '?lang='.JRequest::getCmd('lang','').'&raw=true&option=com_breezingforms&bfReCaptcha=true&form=' . $this->form . '&Itemid=' . JRequest::getInt('Itemid', 0) . '",
