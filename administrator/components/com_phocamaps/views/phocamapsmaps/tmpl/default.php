...
<form action="<?php echo JRoute::_('index.php?option=com_phocamaps&view=phocamapsmaps'); ?>" method="post" name="adminForm" id="adminForm">
...
<thead>
	<tr>
		...
		<th  width="55%"><?php echo JText::_('COM_PHOCAMAPS_PLUGIN_CODE');?></th>
		...
		<th width="10%">
			...
			if ($canOrder && $saveOrder) {
				echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'phocamapsmaps.saveorder');
			} ?>
		</th>
...
$linkEdit	= JRoute::_( 'index.php?option=com_phocamaps&task=phocamapsmap.edit&id='.(int) $item->id );
...
<tr class="row<?php echo $i % 2; ?>">
	...
	<?php if ($item->checked_out) {
		echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'phocamapsmaps.', $canCheckin); 
	} 
	...
	<td><?php echo '{phocamaps view=map|id='.$item->id.'} ' . JText::_('COM_PHOCAMAPS_OR'). '  {phocamaps view=link|id='.$item->id.'|text=Map}';?></td>
	
	<td class="center"><?php echo JHtml::_('jgrid.published', $item->published, $i, 'phocamapsmaps.', $canChange); ?></td>
	<?php
$cntx = 'phocamapsmaps';
echo '<td class="order">';
if ($canChange) {
